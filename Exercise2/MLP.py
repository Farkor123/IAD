...
class neuronlayer():
    def __init__(self, number_of_neurons, number_of_inputs_per_neuron):
        ...
        self.weights = (
            2 * np.asmatrix(np.random.random((number_of_neurons, number_of_inputs_per_neuron))) - 1)
        # bias matrix, outputs x 1
        self.bias = (
            2 * np.asmatrix(np.random.random((number_of_neurons, 1))) - 1)
...
class neuralnetwork():
    ...
    def __tanh(self, x):
        ...
        for _ in range(z):
            for __ in range(y):
                x[_, __] = (m.exp(x[_, __]) - m.exp(-x[_, __])) / \
                    (m.exp(x[_, __]) + m.exp(-x[_, __]))
        return x
    ...
    def forward_propagate(self, input_matrix):
        self.layers[0].input = self.layers[0].weights * \
            input_matrix + self.layers[0].bias
        ...
        for i in range(1, self.layers.size):
            self.layers[i].input = self.layers[i].weights * \
                self.layers[i - 1].output + self.layers[i].bias
    ...
    def compute_errors(self, input_matrix, output_matrix):
        ...
        self.layers[self.layers.size - 1].error = output_matrix - \
            self.layers[self.layers.size - 1].output
        for i in reversed(range(self.layers.size - 1)):
            self.layers[i].error = (np.diagflat(self.__sigmoid_derivative(
                self.layers[i + 1].input)) * self.layers[i + 1].weights).T * self.layers[i + 1].error

    def back_propagate(self, input_matrix, output_matrix, _lambda, _momentum):
        self.compute_errors(input_matrix, output_matrix)
        self.layers[0].v = 2 * _lambda * (self.layers[0].error *
                                          input_matrix.T) + _momentum * self.layers[0].v
        self.layers[0].vb = 2 * \
            _lambda * (self.layers[0].error) + _momentum * self.layers[0].vb
        self.layers[0].weights = self.layers[0].weights + self.layers[0].v
        self.layers[0].bias = self.layers[0].bias + self.layers[0].vb
        for i in range(1, self.layers.size):
            self.layers[i].v = 2 * _lambda * (self.layers[i].error *
                                              self.layers[i - 1].output.T) + _momentum * self.layers[i].v
            self.layers[i].vb = 2 * \
                _lambda * (self.layers[i].error) + \
                _momentum * self.layers[i].vb
